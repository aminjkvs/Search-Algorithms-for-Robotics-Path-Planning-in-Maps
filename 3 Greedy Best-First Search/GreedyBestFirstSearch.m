...
% Priority queue: open list (stores [x, y, h(x)])
openList = [startPoint, heuristic(startPoint, endPoint)];

% Create a figure for visualization
figure;
show(map1); % Show the maze
hold on;
title('Greedy Best-First Search Maze Traversal');
xlabel('X');
ylabel('Y');
...
videoWriter = VideoWriter('C:\Users\Amin\Desktop\Greedy_Best_First_Search.avi'); % Full path for the output video
...
while ~isempty(openList)
    % Sort the open list by heuristic value (ascending order)
    [~, idx] = sort(openList(:, 3)); % Sort by h(x)
    openList = openList(idx, :);
    
    % Take the node with the smallest h(x)
    current = openList(1, 1:2); % Extract the cell
    openList(1, :) = []; % Remove the cell from the open list
    ...
    for i = 1:size(neighbors, 1)
        ...
        % If neighbor is not visited, add it to the open list
        if ~visited(neighbor(1), neighbor(2))
            ...
            openList = [openList; neighbor, heuristic(neighbor, endPoint)];
        end
    end
end
...
function h = heuristic(point, goal)
    h = abs(goal(1) - point(1)) + abs(goal(2) - point(2));
end
