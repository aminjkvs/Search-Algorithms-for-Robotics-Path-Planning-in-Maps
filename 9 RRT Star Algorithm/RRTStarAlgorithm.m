...
rewiringRadius = 5; % Radius for rewiring nearby nodes
...
tree = [startPoint, 0, 0]; % [x, y, parentIndex, costToReach]
foundPath = false;
...
videoFileName = 'C:\Users\Amin\Desktop\RRT_STAR_Path_Planning.avi'; % Specify the output file name
...
for i = 1:maxIterations
    ...
    if newPoint(1) >= 1 && newPoint(1) <= rows && ...
       ...
       occupancyMapSample(newPoint(1), newPoint(2)) == 0
        ...
        % Calculate the cost to reach the new point
        costToReach = tree(nearestIndex, 4) + norm(newPoint - nearestNode);
        
        % Add the new point to the tree
        tree = [tree; newPoint, nearestIndex, costToReach];
        ...
        % Rewiring: Check nearby nodes and rewire if a cheaper path is found
        nearbyNodes = find(sqrt(sum((tree(:, 1:2) - newPoint).^2, 2)) < rewiringRadius);
        for j = 1:length(nearbyNodes)
            nearbyIndex = nearbyNodes(j);
            newCost = costToReach + norm(tree(nearbyIndex, 1:2) - newPoint);
            if newCost < tree(nearbyIndex, 4) % If new path is cheaper
                tree(nearbyIndex, 3) = size(tree, 1); % Update parent
                tree(nearbyIndex, 4) = newCost; % Update cost
                
                % Visualize the rewiring connection
                plot([tree(nearbyIndex, 2), newPoint(2)], ...
                     [rows - tree(nearbyIndex, 1) + 1, rows - newPoint(1) + 1], 'g-', 'LineWidth', 1);
            end
        end
        ...
        if norm(newPoint - goalPoint) < goalThreshold
            foundPath = true;
            tree = [tree; goalPoint, size(tree, 1), costToReach + norm(newPoint - goalPoint)]; % Add goal to the tree
            break;
        end
    end
end
